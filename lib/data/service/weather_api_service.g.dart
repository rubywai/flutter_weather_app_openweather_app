...
class _WeatherApiService implements WeatherApiService {
  ...
  Future<List<CitySearchResult>> searchCity(
      {required city, required limit, required appId}) async {
    ...
    final _result = await _dio.fetch<List<dynamic>>(
        _setStreamType<List<CitySearchResult>>(
  ...
  Future<CurrentWeatherModel> getCurrentWeather(
      {required lat, required lon, required appId, required unit}) async {
    const _extra = <String, dynamic>{};
    final queryParameters = <String, dynamic>{
      r'lat': lat,
      r'lon': lon,
    ...
    final _result = await _dio.fetch<Map<String, dynamic>>(
        _setStreamType<CurrentWeatherModel>(
            Options(method: 'GET', headers: _headers, extra: _extra)
